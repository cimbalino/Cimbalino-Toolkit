                    ...
                    if ($vsInstances = Get-VSSetupInstance) {
                        ...
                        if ($vs) {
                            if ($buildToolsKey -eq 'MSBuildToolsPath32') {
                                ...
                                $frameworkDirs += Join-Path ($vs[0].InstallationPath) MSBuild\Current\Bin\amd64
                            }
                        }
                        ...
                        if (Test-Path -LiteralPath "$root\Microsoft Visual Studio\2019") {
                            if ($buildToolsKey -eq 'MSBuildToolsPath32') {
                                ...
                                $rp = @(Resolve-Path "$root\Microsoft Visual Studio\2019\*\MSBuild\Current\Bin\amd64" -ErrorAction SilentlyContinue)
                            }
